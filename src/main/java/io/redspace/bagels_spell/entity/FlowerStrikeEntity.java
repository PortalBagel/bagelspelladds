...
import io.redspace.bagels_spell.registry.ExampleEntityRegistry;
...
public class FlowerStrikeEntity extends AoeEntity {
    public FlowerStrikeEntity(EntityType<? extends Projectile> pEntityType, Level pLevel) {
        super(pEntityType, pLevel);
        this.setCircular();
    }

    public FlowerStrikeEntity(Level level, LivingEntity owner, float damage, float radius) {
        this(ExampleEntityRegistry.FLOWER_STRIKE.get(), level);
        setOwner(owner);
    ...
    public void tick() {
        if (tickCount == waitTime) {
            ...
            if (!level.isClientSide) {
                ...
                MagicManager.spawnParticles(level, ParticleTypes.CHERRY_LEAVES, center.x, center.y, center.z, 25, 0, 0, 0, .18, false);
                MagicManager.spawnParticles(level, new BlastwaveParticleOptions(ExampleSpellRegistry.FLOWER_STRIKES_SPELL.get().getSchoolType().getTargetingColor(), this.getRadius() * .9f), center.x, center.y, center.z, 1, 0, 0, 0, 0, true);
                ...
                for (Entity entity : entities) {
                    ...
                    if (distanceSqr < explosionRadiusSqr && canHitEntity(entity) && Utils.hasLineOfSight(level, losCenter, entity.getBoundingBox().getCenter(), true)) {
                        ...
                        DamageSources.applyDamage(entity, damage, ExampleSpellRegistry.FLOWER_STRIKES_SPELL.get().getDamageSource(this, getOwner()));
                    }
                }
            }
        ...
        if (level.isClientSide && tickCount < waitTime / 2) {
            ...
            for (int i = 0; i < 3; i++) {
                ...
                level.addParticle(ParticleTypes.CHERRY_LEAVES, vec3.x, vec3.y - .5, vec3.z, motion.x, motion.y, motion.z);
            }
        }
    }
